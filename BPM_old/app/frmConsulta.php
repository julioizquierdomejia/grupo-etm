...
<head>
	...
	function updatePedido(pPedido){
  	...
  	function fillCliente(pObjeto){
	  var changeTipo = "";
	  $.ajaxSetup({cache:true});
	  $.getJSON("selCliente.php","", function(clientes){
		...
		$.each(clientes, function(i, cliente){
			...
			var newOption = "<option value='"+cliente.nombre+"'>"+cliente.nombre+"</option>";
  ...
  $(function() {
	...
	$("#AddProducto").on("click", function() {
		...
		/*
		Lo deshabilito porque está fallando el uso de los botones.
		$('#pedidos').selectable({
			filter:'tbody tr',
		});
		*/
		...
		//$( "#selpromotor" ).selectmenu();
		//$( "#promotor" ).selectmenu();
		$( "#accordion" ).accordion();
		
		
		//$( "#cselmotivo" ).selectmenu();
		//$( "#cselpromotor" ).selectmenu();
...
<ul>
   ...
   <li class='has-sub'><a href='picking.php'>Reportes</a>
    <!--ul>
  ...
  <form id="Pedido">
    <fieldset>
      ...
      <select name="selmotivo" id="selmotivo">
          ...
          <option value="Consignación">Consignaci&oacute;n</option>
	  </select>
	  </div>
      <div class="Cell">
...
<button id="dlgCrearPedido">Crear Pedido</button>
